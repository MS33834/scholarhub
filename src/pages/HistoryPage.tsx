    ...
    <div className="page-fade mx-auto max-w-column px-6 sm:px-8 pt-16 pb-32">
      <header className="border-b border-rule pb-8 flex items-baseline justify-between gap-6 flex-wrap">
        <div>
          ...
          <h1 className="text-4xl sm:text-5xl font-bold text-ink tracking-tight">{t('history.title')}</h1>
        ...
        {historyWithResources.length > 0 && (
          <button
            ...
            className="text-sm font-medium px-4 py-2 border border-rule rounded-lg text-ink-soft hover:text-ochre hover:border-ochre transition-colors"
          >
            {t('history.clearAll')}
          </button>
        )}
      ...
      {historyWithResources.length === 0 ? (
        <div className="mt-20 text-center">
          ...
          <p className="text-3xl font-bold text-ink">{t('history.empty.title')}</p>
        ...
        <section className="mt-10 space-y-3">
          {historyWithResources.map((item) => (
            <div
              key={item.resourceId}
              className="group relative border border-rule rounded-lg p-5 hover:border-ink-soft hover:shadow-md transition-all"
            >
              <Link
              ...
              >
                <div className="flex items-start justify-between gap-4">
                  <div className="flex-1 min-w-0">
                    <h3 className="text-xl font-semibold text-ink leading-snug line-clamp-1 group-hover:text-moss transition-colors">
                      {item.resource!.title}
                    </h3>
