...
export function FilterChips({ label, options, value, onChange }: FilterChipsProps) {
  ...
  return (
    <div className="flex flex-wrap items-center gap-3 overflow-x-auto pb-2">
      <span className="text-mono text-[11px] uppercase tracking-wider2 text-ink-mute w-16 shrink-0">
        {label}
      </span>
...
function Chip({ active, onClick, label }: { active: boolean; onClick: () => void; label: string }) {
  return (
    <button
      onClick={onClick}
      className={`shrink-0 text-sm px-3 py-1.5 border rounded-[2px] transition-colors ${
        active
          ? 'border-moss text-moss'
          : 'border-rule text-ink-soft hover:border-moss/60 hover:text-moss'
      }`}
    >
      {label}
    </button>
